...
- **Bind audio to parameters** — Double-click a parameter port to open the **audio signal picker**. The picker is band-centric: it shows bands (with frequency range) and remappers per band. Create bands for analysis, add remappers to shape the signal, then **connect a remapper** to the parameter (raw band output is not connectable from the picker). Each remapper lists parameters it already drives; click a list item to pan/zoom the node editor to that port (picker stays open). When the port is already connected, double-click opens a **compact** view for that remapper (tweak, see driven parameters, disconnect). From compact mode, **Open full** can reopen the large picker without disconnecting first. **Graph (node) outputs** are not listed in this picker—connect a node output to a parameter by dragging from the node’s output port to the parameter port. Parameters bound to a remapper receive live value at runtime; UI shows “driven” and live value. Legacy graphs that wired band-raw are migrated on load to each band’s default remapper.
...
- **Setup:** Open audio panel (audio toggle in bottom bar, or double-click a parameter port) → add file (e.g. MP3) → configure bands/remappers. Bind to a parameter by double-clicking the parameter port → picker shows bands and remappers → connect a remapper → preview reacts.
