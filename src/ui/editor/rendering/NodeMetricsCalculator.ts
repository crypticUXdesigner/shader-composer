...
export class NodeMetricsCalculator {
  ...
  private calculateMetrics(node: NodeInstance, spec: NodeSpec): NodeRenderMetrics {
    ...
    if (Object.keys(spec.parameters).length > 0 && !isColorPickerOnly) {
      ...
      if (isBezierNode) {
        ...
        // Use explicit columns from layout when available; otherwise derive from groups
        let maxColumns = 1;
        const layoutCols = this.getMaxExplicitColumnsFromLayout(layout);
        if (layoutCols != null) {
          maxColumns = layoutCols;
        } else {
        ...
        // Apply extraColumns from layout (e.g. wide custom nodes that need extra space)
        const extraColumns = layout.extraColumns ?? 0;
        // minColumns forces column count when set (e.g. color map nodes need 3 columns)
        const effectiveColumns = layout.minColumns != null
          ? layout.minColumns
          : maxColumns + Math.max(0, extraColumns);
  ...
  /** Get max explicit columns from layout elements (grid with layout.columns as number). */
  private getMaxExplicitColumnsFromLayout(layout: { elements?: Array<{ type?: string; layout?: { columns?: number | string } }> }): number | null {
    let max: number | null = null;
    for (const el of layout.elements ?? []) {
      if (el.type === 'grid' && el.layout && typeof el.layout.columns === 'number') {
        max = max == null ? el.layout.columns : Math.max(max, el.layout.columns);
      }
    }
    return max;
  }
