...
export class BodyFlexboxLayout {
   ...
   * Slot gap must match NodeBody content gap (param-grid-gap) so layout height
   * matches the DOM. Previously used bodyPadding (18px) which over-estimated height.
   */
  private getBodyFlexboxProperties(): FlexboxProperties {
    const slotGap = getCSSVariableAsNumber('param-grid-gap', 6);
