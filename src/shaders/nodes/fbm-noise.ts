...
float fbm(vec2 p, float time) {
  ...
  for (int i = 0; i < 8; i++) {
    if (float(i) >= $param.fbmOctaves) break;
    ...
    frequency *= $param.fbmLacunarity;
    amplitude *= $param.fbmGain;
  }
...
`,
  ...
  $output.out += fbm($input.in * $param.fbmScale, fbmTime) * $param.fbmIntensity;
`
};
