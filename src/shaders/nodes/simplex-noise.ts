...
export const simplexNoiseNodeSpec: NodeSpec = {
  id: 'simplex-noise',
  category: 'Patterns',
...
float simplexFbm(vec2 p, float time) {
  ...
  for (int i = 0; i < 8; i++) {
    if (float(i) >= float($param.simplexOctaves)) break;
    ...
    frequency *= $param.simplexLacunarity;
    amplitude *= $param.simplexGain;
  }
...
`,
  mainCode: `
  float simplexTime = ($time + $param.simplexTimeOffset) * $param.simplexTimeSpeed;
  float value = simplexFbm($input.in * $param.simplexScale, simplexTime);
  $output.out += value * $param.simplexIntensity;
`
};
