...
export function testResetNodeParametersToDefaults(): void {
  ...
  const next = resetNodeParametersToDefaults(graph, 'n-noise', noiseSpecs);
  const node = findNode(next, 'n-noise');
  assert(node !== undefined, 'node should exist');
  assertEqual(node.parameters.noiseScale, 2.0, 'noiseScale default');
  assertEqual(node.parameters.noiseOctaves, 4, 'noiseOctaves default');
  assertEqual(node.parameters.noiseIntensity, 0.5, 'noiseIntensity default');
  assert(node.parameterInputModes === undefined, 'parameterInputModes should be cleared');
  assert(next !== graph, 'graph reference should change');
}
...
describe('data-model', () => {
  ...
  it('resetNodeParametersToDefaults', testResetNodeParametersToDefaults);
});
