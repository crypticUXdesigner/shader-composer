/**
 * Mirrors WebGPU still export: `compiler.compile(graph, audioSetup, { backend: 'webgpu' })`
 * (`WebGpuExportRenderPath`). Locks deterministic replay + bypass execution-order invariants for CI.
 */
...
} from './nodePowerBypassExportCompilationGateAssertions';

function buildNodeSpecsMap(): Map<string, NodeSpec> {
  return new Map(nodeSystemSpecs.map((s) => [s.id, s]));
}

describe('Node Power bypass — image export compile gate (WebGPU)', () => {
  ...
  it('Rule A (rotate bypassed): deterministic compile; bypassed node out of execution order', () => {
    assertBypassExportCompileGate(compiler, bypassRuleAGraph(), ['n-rotate'], 'rule-a');
  });

  it('Rule B (noise bypassed): deterministic compile; bypassed node out of execution order', () => {
    assertBypassExportCompileGate(compiler, bypassRuleBPatternGraph(), ['n-noise'], 'rule-b');
  });
});
